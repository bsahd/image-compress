    ...
    for (let y = 0; y < height; y += 8) {
      for (let x = 0; x < width; x += 8) {
        ...
        const nblock = blockyuv.map((x, yi) =>
          x.map(([cy, cu, cv], xi) => [
            blockdrangey < COMPRESS_LEVEL / 2 ? 0 : (cy - blockminy) / blockdrangey,
        ...
        const nblock4bn = nblock4b.map((x) => x.map(([r, g, b]) => (r * 4 + g) * 4 + b));
        const corners = [blockyuv[0][0], blockyuv[0][7], blockyuv[7][0], blockyuv[7][7]]
          .map(([cy, cu, cv]) => [
          ...
          .map(([cy, cu, cv], y) => {
            const qy = blockdrangey < COMPRESS_LEVEL / 2 ? Math.floor(cy * 15.9) : 0;
